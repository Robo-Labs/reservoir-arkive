...
import { Context, SECONDS_PER_YEAR, toNumber } from "./util.ts";
...
const updateSnapshot = async (ctx: Context, now: number, period: string) => {
  ...
  return await Promise.all(pairs.map(async (pair) => {
    ...
    const volume0 = swaps.reduce(
      (acc: number, swap: ISwap) => acc += swap.amount0In + swap.amount0Out,
      0,
    );
    const volume1 = swaps.reduce(
      (acc: number, swap: ISwap) => acc += swap.amount1In + swap.amount1Out,
      0,
    );
    ...
    const [aaveSnapshot0, aaveSnapshot1] = await Promise.all([
      AaveSnapshot.findOne({ underlyingAddress: pair.token0 }).sort({
        timestamp: -1,
      }),
      AaveSnapshot.findOne({ underlyingAddress: pair.token1 }).sort({
        timestamp: -1,
      }),
    ]);
    ...
    const managed = await client.multicall({
      contracts: [
        {
          abi: AaveAssetManagerAbi,
          address: ASSET_MANAGER,
          functionName: "getBalance",
          args: [pair.address as Address, pair.token0 as Address],
        },
        {
          abi: AaveAssetManagerAbi,
          address: ASSET_MANAGER,
          functionName: "getBalance",
          args: [pair.address as Address, pair.token1 as Address],
        },
      ],
    ...
    const managedApy = liquidityRate0 * (managed0 / pair.reserve0) +
    ...
    // update pair.tvlUSD
    pair.tvlUSD = (pair.reserve0 * token0.priceUSD) +
      (pair.reserve1 * token1.priceUSD);
    await pair.save();
  }));
};
...
export const SnapshotHandler: BlockHandler = async (
  ctx: Context,
): Promise<void> => {
